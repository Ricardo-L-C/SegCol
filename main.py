...
def parse_args():
    ...
    parser.add_argument("--data_dir", default=str(dataset_path), help="Path to the train/test data root directory")
    ...
    parser.add_argument("--pretrain_dump", default=str(pretrain_path), help="Path of pretrained CIT network dump.")
    parser.add_argument("--tag_dump", default=str(tag_dump_path), help="Path of tag dictionary / metadata pickle file.")
...
def validate_args(args):
    ...
    if args.load != "":
        assert Path(args.load).exists(), "cannot find network dump file"
    assert Path(args.pretrain_dump).exists(), "cannot find pretrained CIT network dump file"
    assert Path(args.tag_dump).exists(), "cannot find tag metadata pickle file"
