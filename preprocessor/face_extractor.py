...
if __name__ == "__main__":
    desc = "Tag2pix face extractor. Not used in Seg colorization."
    parser = argparse.ArgumentParser(description=desc)

    parser.add_argument("--dataset_path", type=Path, default="./dataset", help="path to dataset directory")
    parser.add_argument("--add_all", action="store_true", help="Move every face image in the `temp_faces` to `rgb_train`. It also crops face images from sketches and add them to train set")

    args = parser.parse_args()

    if args.add_all:
        move_all(args.dataset_path)
    else:
        detect_all(args.dataset_path)